...
featuredProviders.innerHTML = providers.map((provider) => `<article class="provider-horizontal-card"> <div class="provider-card-avatar"> ${provider.photo && provider.photo.trim() !== "" ?`<img src="${provider.photo}" alt="${escapeHtml(provider.name)}" />`:`<div class="avatar-placeholder-small">${escapeHtml((provider.name || “HL”).slice(0, 2).toUpperCase())}</div>`} </div> <div class="provider-card-info"> <div class="provider-card-header"> <div class="provider-name-rating"> <h3 class="provider-name">${escapeHtml(provider.name)}</h3> <div class="provider-rating"> ${createRatingStars(provider.rating)} <span>(${provider.review_count || 0})</span> </div> </div> ${provider.verified ? '<span class="verified-badge">&#10003; Verified</span>' : ""} </div> <div class="provider-location"> &#128205; ${escapeHtml(formatLocation(provider) || "Location pending")} </div> <p class="provider-bio">${escapeHtml(provider.bio || "No bio added yet.")}</p> <div class="provider-tags"> <span class="provider-tag">${escapeHtml(provider.skill || "Skilled worker")}</span> <span class="provider-tag">${escapeHtml(provider.category || "General")}</span> </div> <button class="view-profile-btn" onclick="window.location.href='/profile.html?id=${provider.id}'">View Profile &rarr;</button> </div> </article>`).join(””);
} catch (error) {
...
// ── Work photos: direct file picker capture (fixes registration bug) ──
