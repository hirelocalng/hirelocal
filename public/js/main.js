...
<article class="result-card admin-card">
  <div class="result-card-head">
    <div style="display:flex;align-items:center;gap:0.75rem;">
      ${provider.photo && provider.photo.trim() !== "" ? `<img src="${provider.photo}" alt="Profile photo" style="width:48px;height:48px;border-radius:50%;object-fit:cover;" />` : `<span class="avatar-dot">${escapeHtml((provider.name || "HL").slice(0, 2).toUpperCase())}</span>`}
      <div>
...
<article class="photo-tile">
  <button type="button" class="remove-photo-btn" data-index="${index}" style="position:absolute;top:4px;right:4px;background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;width:24px;height:24px;cursor:pointer;z-index:1;">×</button>
  <img src="${photo}" alt="Work photo ${index + 1}" style="width:100%;height:120px;object-fit:cover;" />
...
collectedWorkPhotos.splice(index, 1);
renderWorkPhotos();
});
});
};
...
id_photo: idPhotos[0],
work_photos: collectedWorkPhotos,
