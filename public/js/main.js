...
// Renders provider avatar: photo if available, otherwise initials
...
return `<span class="avatar-dot">${initials}</span>`;
};
...
featuredProviders.innerHTML = providers.map((provider) => `<article class="profile-preview"> <div class="profile-preview-top"> ${renderProviderAvatar(provider)} <div> <h3>${escapeHtml(provider.name)}</h3> <p>${escapeHtml(formatLocation(provider) || "Location pending")}</p> </div> </div> <p>${escapeHtml(provider.bio || "No bio added yet.")}</p> <a class="button secondary full" href="/profile.html?id=${provider.id}">Open Profile</a> </article>`).join(””);
...
// Track pending photo chosen by owner in edit form
let editPendingPhoto = null;

// Wire up the profile picture file picker preview in the edit form
...
// ── KEY FIX: show edit section ONLY to the profile owner ──────────────────
// Checks localStorage for a logged-in provider whose numeric ID matches
// the ?id= param in the URL. Visitors, other providers, and logged-out
// users will always have editSection hidden (display:none via CSS).
...
if (String(stored.id) === String(loadedProviderId)) {
  // Owner is viewing their own profile — reveal the edit form
  editSection.classList.add("owner-visible");

  // Pre-fill name and skill from stored data
  ...
  // Show current profile picture as preview if they have one
...
// ── Handle edit-profile form submission (owner only) ──────────────────────
...
const payload = {
  // Keep all existing fields so the API update doesn't wipe them out
...
// Include photo if owner picked a new one, otherwise keep existing
if (editPendingPhoto) {
...
try {
  ...
  if (data.success) {
    ...
    // Reload to reflect changes on the public profile
    loadProfile();
  } else {
...
// ── Load and render public profile ────────────────────────────────────────
...
```
  if (profile) {
    // Render profile picture if available, otherwise show initials
    const avatarHtml = provider.photo
      ? `<img
           src="${provider.photo}"
           alt="${escapeHtml(provider.name)} profile photo"
           style="width:96px;height:96px;border-radius:50%;object-fit:cover;display:block;margin:0 auto 1rem;"
         />`
  ...
  // After profile data loads, check if the viewer is the owner
  maybeRevealEditSection(provider.id);

  // Also refresh the pre-fill with the latest server data for the edit form
  if (editSection?.classList.contains("owner-visible")) {
    ...
    // Update stored provider with freshest data from API
...
// Bind photo picker preview on dashboard update form
...
```
  ...
  // Show profile photo in dashboard summary panel
  const avatarHtml = provider.photo
    ? `<img src="${provider.photo}" alt="Profile photo"
         style="width:80px;height:80px;border-radius:50%;object-fit:cover;display:block;margin-bottom:0.75rem;" />`
    : `<div class="avatar-dot" style="width:80px;height:80px;font-size:1.75rem;display:flex;align-items:center;justify-content:center;border-radius:50%;background:#e5e7eb;margin-bottom:0.75rem;">
         ${escapeHtml((provider.name || "HL").slice(0, 2).toUpperCase())}
       </div>`;
  ...
  // Show current photo in dashboard photo preview area
  if (photoPreview && !pendingPhoto) {
    photoPreview.innerHTML = provider.photo
      ? `<article class="photo-tile single">
           <img src="${provider.photo}" alt="Current profile photo"
             style="width:96px;height:96px;border-radius:50%;object-fit:cover;" />
           <span>Current photo</span>
         </article>`
...
// Include photo in dashboard update payload
...
// Send new photo if picked, otherwise keep existing
if (pendingPhoto) {
...
// Bind profile picture preview on register form
